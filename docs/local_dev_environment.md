...
> One of the requirements `dictionaryutils==3.4.0` may not be available on through pip, so you can try with a lower available version.  In this case, update [requirements.txt](https://github.com/uc-cdis/sheepdog/blob/master/requirements.txt) to use `dictionaryutils==3.2.0`.  You may also need to update the requirement for `gen3authz==0.2.1` to `gen3authz==0.4.0`.
...
> You may need to update the virtual environment activation step in the bash script before running it.
